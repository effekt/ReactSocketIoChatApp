            ...
            <Scrollbars ref="messageScrollbar">
              {
                this.state.messages.map((message, i) => {
                  if (message.by)
                    return (<div className="clearfix" key={i}>{String(message.by) !== String(this.state.username) ? '<small>' + message.by + '</small>' : ''}<br /><div className={String(message.by) === String(this.state.username) ? "message-me" : "message"}>{message.message}</div></div>)
                  else
